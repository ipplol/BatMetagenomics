...
void ReadData()
{
    ...
    while (getline(readraw, line))
    {
        ...
        for (j = 5; j < line1.size(); j++)
        {
            ...
            if (linemc[0][0] == '-' && a.DELDepth < stoi(linemc[1]))
            {
                string tmps = linemc[0].substr(1, linemc[0].size() - 1);
                a.DEL = "DEL-" + to_string(tmps.size()) + "-" + tmps; a.DELDepth = stoi(linemc[1]); a.DELForwardReads = stoi(linemc[5]); a.DELBackwardReads = stoi(linemc[6]);
            }
            if (linemc[0][0] == '+' && a.INSDepth < stoi(linemc[1]))
            {
                string tmps = linemc[0].substr(1, linemc[0].size() - 1);
                a.INS = "INS-" + to_string(tmps.size()) + "-" + tmps; a.INSDepth = stoi(linemc[1]); a.INSForwardReads = stoi(linemc[5]); a.INSBackwardReads = stoi(linemc[6]);
            }
        }
